...
int MainWindow::imageWidth = 0;
int MainWindow::imageHeight = 0;
...
MainWindow::MainWindow(QWidget *parent)
    ...
    layout->addWidget(imageLabel, 0, 0, 1, 2, Qt::AlignCenter);

	imageWidth  = LeptonThread::FrameWidth  * SCALE;
	imageHeight = LeptonThread::FrameHeight * SCALE;
    ...
    QPushButton *ffcButton = new QPushButton("FFC");
    layout->addWidget(ffcButton, 1, 1, Qt::AlignCenter);
    connect(ffcButton, SIGNAL(clicked()), this, SLOT(ffcManual()));
...
void MainWindow::updateImage(unsigned short *data, int minValue, int maxValue){
    ...
    for (int y = 0; y < LeptonThread::FrameHeight; ++y) {
        for (int x = 0; x < LeptonThread::FrameWidth; ++x) {
		    ...
		    //qDebug() << "TEST: x, y, maxValue, minValue, baseValue =" << x << y << maxValue << minValue << baseValue;
            int scaledValue = 256*(baseValue - minValue)/diff; // map value to interval [0, 256), and set the pixel to its color value above
            if (scaledValue > 255) {
            	rangeErrors++;
				//qDebug() << "RangeError: x, y, maxValue, minValue, baseValue, scaledValue =" 
				//             << x << y << maxValue << minValue << baseValue << scaledValue;
            	scaledValue = 255;
            }
    ...
    if (rangeErrors > 0) {
		qDebug() << "RangeErrors in frame: =" << rangeErrors;
    }
...
void MainWindow::ffcManual() {
	qDebug() << "Manual FFC requested";
	thread->ffcCtrl();
}
