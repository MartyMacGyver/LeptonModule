...
unsigned int LeptonThread::speed = 18000000;
unsigned short LeptonThread::delay = 0;
bool LeptonThread::ffcCtrlQueued = false;
...
void LeptonThread::run() {    
    ...
    while (true) {
		...
		// Signal the main thread to update the image
		...
		if (ffcCtrlQueued) {
		    ...
		    LEP_SYS_FFC_SHUTTER_MODE_OBJ_T _ffc;
		    LEP_GetSysFfcShutterModeObj(&_port, &_ffc);
		    qDebug() << "Manual FFC data" << _ffc.shutterMode;
		    _ffc.desiredFfcPeriod = 300000;
		    //_ffc.desiredFfcPeriod = 15000;
		    LEP_SetSysFfcShutterModeObj(&_port, _ffc);
		    LEP_SYS_STATUS_E _ffcStatus;
			LEP_GetSysFFCStatus(&_port, &_ffcStatus);
		    qDebug() << "Manual FFC status" << _ffcStatus;
...
void LeptonThread::ffcCtrl() {
	qDebug() << "Manual FFC ACK";
	ffcCtrlQueued = true;
}
